...
Updated to v7: uses `@prisma/adapter-pg` with `pg` Pool for direct database connections. Config is in `prisma.config.ts`; `schema.prisma` no longer contains the `url` property.
